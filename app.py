...
# 2. CONEXÕES (Chave Nova Aplicada)
...
def carregar_memoria():
    if st.session_state.supabase:
        try:
            res = st.session_state.supabase.table("messages").select("*").order("created_at", desc=False).limit(20).execute()
...
# 3. INTERAÇÃO (Finch/Sexta-Feira Mode)
if prompt := st.chat_input("Diga..."):
    ...
    with st.chat_message("assistant", avatar="🕵️‍♂️"):
        try:
            system_msg = {
                "role": "system", 
                "content": "Você é o Athos (Finch/Sexta-Feira). Seja breve, sutil e elegante. Decida pelo usuário para reduzir seu cansaço mental. Guarde cada detalhe dito para nunca repetir perguntas. Não interrogue, apenas acompanhe."
            }
            
            # Filtro para garantir que só enviamos texto para a API
            history = [{"role": m["role"], "content": str(m["content"])} for m in st.session_state.messages[-12:]]
